...
def load_pieces(folder):
    ...
    for idx, item in enumerate(edges):
        ...
        top, right, bottom, left = extract_sides(item["image"])
        ot, orr, ob, ol = extract_sides_color(original)
...
# ==========================================================
def total_error(arr, weight_pixel=0.001, weight_shape=0.0005):
    TL, TR, BL, BR = arr
    ERR = 0

    # 1️⃣ Masked HSV histogram similarity
    ...
    # 2️⃣ Pixel-by-pixel difference (could be added with small weight)
    # e.g.:
    # ERR += weight_pixel * edge_distance_pixels(TL["right"], TR["left"])
    # ...
...
def compute_best_side_costs(pieces):
    """
    For each piece i, compute the best possible match cost for each side:
...
def choose_first_piece_top_left(pieces):
    """
    Choose a top-left corner candidate:
    - bad TOP side  (no good match above)
    - bad LEFT side (no good match left)
    'Bad' means best match cost > threshold (e.g., 75th percentile).
    """
    best_top, best_right, best_bottom, best_left = compute_best_side_costs(pieces)

    # collect all best-side costs to estimate what's "large"
    all_vals = np.concatenate([best_top, best_right, best_bottom, best_left])
    thr = np.percentile(all_vals, 75)  # sides worse than this are considered "bad"
    ...
    for i in range(len(pieces)):
        ...
        if bad_top and bad_left:
            # score: how strong a corner it is (bigger = more "bad")
            score = best_top[i] + best_left[i]
            candidates.append((score, i))

    if candidates:
        # choose the piece with the worst (largest) sum of top+left
        candidates.sort(reverse=True)
        start_idx = candidates[0][1]
    else:
        # fallback: just pick the piece with largest (top+left)
...
# ==========================================================
def optimize_2x2_horizontal(pieces):
...
if __name__ == "__main__":
    ...
    output_folder = r"D:\Semester 5\Image processing\Project\Image_Milestone2\output_to_collect\assembled_masked"
    ...
    for i in range(110):
        ...
        # 🔹 NEW: choose first piece as top-left corner candidate (for analysis / future NxN)
        start_idx, (btop, bright, bbottom, bleft), thr = choose_first_piece_top_left(pieces)
        print("Corner-based start (top-left) piece:", pieces[start_idx]["edge_path"])
        print("Threshold for 'bad' side:", thr)

        # still using full brute force for 2x2:
        TL, TR, BL, BR = optimize_2x2_horizontal(pieces)
